...
namespace Programmerare.ShortestPaths.Core.Parsers
{
    ...
    //	    </outputExpected>
		    ...
		    var lListOfPaths = pathParser.FromStringToListOfPaths("13 A B D\r\n" + 
				    "15 A C D\r\n" + 
				    "21 A B C D");
		    Assert.NotNull(lListOfPaths);
		    Assert.Equal(3, lListOfPaths.Count);
		    ...
		    Assert.Equal(13.0, path1.TotalWeightForPath.WeightValue, 8);
		    Assert.Equal(15.0, path2.TotalWeightForPath.WeightValue, 8);
		    Assert.Equal(21.0, path3.TotalWeightForPath.WeightValue, 8);
		
		    var edgesForPath1 = path1.EdgesForPath;
		    var edgesForPath2 = path2.EdgesForPath;
		    var edgesForPath3 = path3.EdgesForPath;
		    Assert.NotNull(edgesForPath1);
		    Assert.NotNull(edgesForPath2);
		    Assert.NotNull(edgesForPath3);
		    Assert.Equal(2, edgesForPath1.Count);
		    Assert.Equal(2, edgesForPath2.Count);
		    Assert.Equal(3, edgesForPath3.Count);

		    // edgesForPath1 "13 A B D" means path "A -> B" and "B -> D"
		    Assert.Equal(pathParser.GetEdgeIncludingTheWeight("A", "B"), edgesForPath1[0]);
		    Assert.Equal(pathParser.GetEdgeIncludingTheWeight("B", "D"), edgesForPath1[1]);

		    // edgesForPath2 // 15 A C D
		    Assert.Equal(pathParser.GetEdgeIncludingTheWeight("A", "C"), edgesForPath2[0]);
		    Assert.Equal(pathParser.GetEdgeIncludingTheWeight("C", "D"), edgesForPath2[1]);
		
		    // 21 A B C D
		    Assert.Equal(pathParser.GetEdgeIncludingTheWeight("A", "B"), edgesForPath3[0]);
		    Assert.Equal(pathParser.GetEdgeIncludingTheWeight("B", "C"), edgesForPath3[1]);
		    Assert.Equal(pathParser.GetEdgeIncludingTheWeight("C", "D"), edgesForPath3[2]);
	    }
	    ...
	    public void test_fromStringToPath_Generic_and_fromPathToString() {
		    ...
		    Assert.Equal(inputPathString, outputPathString);
	    }
